from utils import get_network, get_optimizer, get_lr_scheduler, get_loss_functions
...
class Learner(Logger):
    ...
    def calculate_fps_throughput(self):
      ...
      throughput_fps['learner'] = (self.config.num_unroll_steps + 1) * self.config.batch_size * self.training_step / (time_now - self.time_at_start)
    ...
    def train_network(self):
      ...
      while self.training_step < self.config.training_steps:
        ...
        while len(not_ready) > 0:
            ...
            if self.config.evaluation_frequency and n % self.config.evaluation_frequency == 0:
              self.evaluator.evaluate_network.remote(n, self.network.get_weights())
    ...
    def perform_update(self, batch):
      ...
      observations = self.config.to_torch(observations, self.device, scale=255.0)
      ...
      for i, action in enumerate(zip(*actions), 1):
        ...
        loss = self.scalar_loss_fn(reward.squeeze(), target_rewards[:, i])
        reward_loss += loss

        loss = self.scalar_loss_fn(value.squeeze(), target_values[:, i])
        value_loss += loss

        loss = self.policy_loss_fn(policy_logits.squeeze(), target_policies[:, i])
        policy_loss += loss
